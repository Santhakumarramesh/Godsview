...
class BacktestEngine {
  run(strategy: MockStrategy, historicalData: { bars: number }): BacktestResult {
    ...
    // Promotion criteria: win rate > 50%, profit factor > 1.5, enough trades, risk per trade <= 5%
    const passesPromotion = winRate > 0.5 && profitFactor > 1.5 && totalTrades > 20 && strategy.riskPerTrade <= 0.05;
...
class TradingPipeline {
  ...
  runBacktest(strategy: MockStrategy): BacktestResult {
    const result = this.backtester.run(strategy, { bars: 2000 });
...
describe("Trading Pipeline — Failure Path (Risk Gate Blocks)", () => {
  ...
  it("F2: Risk gate BLOCKS order due to bad risk parameters", () => {
    const blockedTrade = {
      symbol: "BTCUSD",
      entryPrice: 67000,
      stopLoss: 50000, // Very wide SL = 25% risk (exceeds 15% max drawdown)
      takeProfit: 67100,
      quantity: 1.0, // Way too large (exceeds 5% max position size)
      riskPerTrade: 0.25,
    };
  ...
  it("F3: Kill switch activates when risk gate blocks trade", async () => {
    ...
    const badTrade = {
      symbol: "BTCUSD",
      entryPrice: 67000,
      stopLoss: 50000, // 25% risk > 15% max drawdown
      takeProfit: 67100,
      quantity: 1.0,
      riskPerTrade: 0.25,
    };
