...
export default function EntryPlanner() {
  ...
  const { data: setup, isLoading: loadingSetup, error: errorSetup } = useQuery({
  ...
  const { data: account, isLoading: loadingAccount, error: errorAccount } = useQuery({
  ...
  const { data: riskCheck, isLoading: loadingRisk, error: errorRisk } = useQuery({
    queryKey: ["risk", "check"],
    queryFn: async () => {
  ...
  return (
    ...
    >
      ...
      {(loadingSetup || loadingAccount || loadingRisk) && (
        <div style={{ textAlign: "center", padding: "40px", color: "#767576" }}>Loading data...</div>
      )}

      {(errorSetup || errorAccount || errorRisk) && (
