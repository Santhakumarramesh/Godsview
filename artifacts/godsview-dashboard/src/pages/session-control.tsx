...
export default function SessionControlPage() {
  ...
  const { data: sessionsData, isLoading: loadingSessions, error: errorSessions } = useQuery({
    queryKey: ["sessions"],
    queryFn: () => fetch(`${API}/api/sessions`).then((r) => r.json()),
    refetchInterval: 5000,
  });

  const { data: executionData, isLoading: loadingExecution, error: errorExecution } = useQuery({
