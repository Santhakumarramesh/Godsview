    ...
    function connect() {
      ...
      try {
        ...
        es.addEventListener("agent_report", (e) => {
          ...
          try {
            const data = JSON.parse(e.data) as BrainSSEEvent;
            const report = (data.report ?? data.payload) as unknown as AgentReportDTO;
            const key = `${data.agentId}:${data.symbol}`;
            setAgents((prev) => {
              const next = new Map(prev);
              next.set(key, {
        ...
        es.addEventListener("brain_decision", (e) => {
          ...
          try {
            const data = JSON.parse(e.data) as BrainSSEEvent;
            const decision = (data.decision ?? data.payload) as unknown as BrainDecisionDTO;
