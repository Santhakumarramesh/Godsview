...
// ── Direct injection (Phase 12A — from account WebSocket stream) ──────────────
// Allows the account stream to push a fill immediately instead of waiting for
// the next 10s poll. The polling loop deduplicates via processedFillIds.

export async function _injectFill(fill: {
  id?: string;
  order_id?: string;
  symbol?: string;
  side?: string;
  ...
  const fillId = fill.id ?? `${fill.order_id}-${fill.transaction_time}`;
  if (processedFillIds.has(fillId)) return; // already processed by poll
  processedFillIds.add(fillId);

  const side = (fill.side ?? "buy") as "buy" | "sell";
  const qty = Number(fill.qty) || 0;
  const price = Number(fill.price) || 0;
  const symbol = fill.symbol ?? "UNKNOWN";
  ...
  if (cb) {
    ...
    if (isClosing && price > 0 && cb.avg_entry > 0) {
      matched = true;
      const priceDiff = cb.direction === "long" ? price - cb.avg_entry : cb.avg_entry - price;
      pnl = priceDiff * qty;
      realizedPnlToday += pnl;
      dailyPnl.set(realizedPnlToday);
      reduceCostBasis(symbol, qty);
      logger.info({ symbol, side, qty, price, pnl: pnl.toFixed(2), source: "ws" }, "WS fill reconciled with PnL");
      try {
        const { recordRealizedPnl } = await import("./drawdown_breaker");
        recordRealizedPnl(pnl, symbol);
      } catch { /* breaker not loaded */ }
