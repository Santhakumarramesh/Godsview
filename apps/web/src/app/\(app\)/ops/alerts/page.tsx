...
const mockAlerts: Alert[] = [
  {
    id: "1",
    timestamp: "2024-04-20 14:35:22",
    type: "Risk",
    severity: "critical",
    title: "Drawdown Threshold Breach",
    message: "Portfolio drawdown exceeded -5% weekly threshold. Auto-derisking rules activated.",
    acknowledged: false,
  },
  {
    id: "2",
    timestamp: "2024-04-20 14:20:15",
    type: "Market",
    severity: "warning",
    title: "High Volatility Detected",
    message: "VIX exceeded 25. Market regime changed to volatile. Adjust position sizes accordingly.",
    acknowledged: false,
  },
  {
    id: "3",
    timestamp: "2024-04-20 14:15:48",
    type: "Orders",
    severity: "warning",
    title: "Large Order Execution",
    message: "Order 12847: 5000 shares AAPL executed at $182.45. Slippage: 0.02%",
    acknowledged: true,
  },
  {
    id: "4",
    timestamp: "2024-04-20 14:10:32",
    type: "Market",
    severity: "info",
    title: "Breakout Signal Generated",
    message: "NVDA broke above $900 resistance. Trend following strategy activated.",
    acknowledged: true,
  },
  {
    id: "5",
    timestamp: "2024-04-20 13:55:17",
    type: "Infrastructure",
    severity: "critical",
    title: "Data Feed Latency High",
    message: "Market data latency exceeded 500ms. Real-time analysis affected.",
    acknowledged: false,
  },
  {
    id: "6",
    timestamp: "2024-04-20 13:40:05",
    type: "Market",
    severity: "info",
    title: "Support Level Tested",
    message: "TSLA tested daily support at $240. Holding strong.",
    acknowledged: true,
  },
  {
    id: "7",
    timestamp: "2024-04-20 13:25:33",
    type: "Orders",
    severity: "warning",
    title: "Partial Fill",
    message: "Order 12845: 2000 shares filled, 3000 shares pending. Queue position: 5th.",
    acknowledged: false,
  },
  {
    id: "8",
    timestamp: "2024-04-20 13:10:19",
    type: "Infrastructure",
    severity: "info",
    title: "System Health Check",
    message: "All systems operational. CPU: 45%, Memory: 62%, Network: 12ms latency.",
    acknowledged: true,
  },
  {
    id: "9",
    timestamp: "2024-04-20 12:55:42",
    type: "Market",
    severity: "warning",
    title: "Correlation Spike",
    message: "Portfolio correlation increased to 0.82. Diversification recommended.",
    acknowledged: false,
  },
  {
    id: "10",
    timestamp: "2024-04-20 12:40:11",
    type: "Data",
    severity: "info",
    title: "Quote Update Received",
    message: "Updated quotes for 150 symbols. Last update: 12:40:08 UTC.",
    acknowledged: true,
  },
];
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-xs font-medium uppercase mb-2">Total Alerts</p>
            <p className="text-2xl font-bold text-white">{alerts.length}</p>
          </div>
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
            <p className="text-red-300 text-xs font-medium uppercase mb-2">Critical</p>
            <p className="text-2xl font-bold text-red-400">
              {alerts.filter((a) => a.severity === "critical").length}
            </p>
          </div>
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
            <p className="text-amber-300 text-xs font-medium uppercase mb-2">Warning</p>
            <p className="text-2xl font-bold text-amber-400">
              {alerts.filter((a) => a.severity === "warning").length}
            </p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
            <p className="text-blue-300 text-xs font-medium uppercase mb-2">Info</p>
            <p className="text-2xl font-bold text-blue-400">
              {alerts.filter((a) => a.severity === "info").length}
            </p>
          </div>
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-xs font-medium uppercase mb-2">Acknowledged</p>
            <p className="text-2xl font-bold text-green-400">
              {alerts.filter((a) => a.acknowledged).length}
            </p>
          </div>
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-xs font-medium uppercase mb-2">Pending</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {Object.entries(alertsByCategory).map(([category, count]) => (
            <div
              key={category}
              className={`p-4 rounded-lg border cursor-pointer transition-all ${getTypeColor(category)} hover:opacity-80`}
            >
        ...
        <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4 flex flex-wrap gap-4 items-center">
          ...
          <div className="flex gap-2">
            ...
            <select
              ...
              className="px-3 py-1 bg-slate-800 border border-slate-600 rounded text-sm text-white focus:outline-none focus:border-amber-400"
            >
          ...
          <button className="ml-auto px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 border border-slate-600 rounded text-sm font-semibold transition-all">
            Clear Filters
          </button>
        </div>
                    ...
                    >
                      <td className="text-center py-4 px-4">
                        <input
                          ...
                          onChange={() => handleAcknowledge(alert.id)}
                          className="rounded border-slate-600 cursor-pointer"
                        />
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <span
                          className={`px-3 py-1 rounded text-xs font-bold border w-fit inline-block ${getTypeColor(
                            alert.type
                          )}`}
                        >
                          {alert.type}
            ...
            <div className="space-y-3">
              {alerts
                ...
                .map((alert) => (
                  <div
                    key={alert.id}
                    className="bg-red-500/5 border border-red-500/20 rounded p-4 flex items-start justify-between"
                  >
                    ...
                    <button
                      onClick={() => handleAcknowledge(alert.id)}
                      className="ml-4 px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/30 rounded font-semibold transition-all whitespace-nowrap"
                    >
                      Acknowledge
                    </button>
                  </div>
