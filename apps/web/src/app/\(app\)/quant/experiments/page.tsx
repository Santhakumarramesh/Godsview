...
const mockExperiments: Experiment[] = [
  {
    ...
    bestResult: 2.34,
    results: [
      { metric: "Win Rate", value: 62.5 },
      { metric: "Profit Factor", value: 2.34 },
      { metric: "Sharpe Ratio", value: 1.87 },
      { metric: "Max Drawdown", value: -8.3 },
    ],
    startedAt: "2024-04-15 09:00",
    completedAt: "2024-04-15 14:30",
  },
  {
    id: "2",
    name: "Volume Breakout Grid",
    ...
    results: [
      ...
      { metric: "Profit Factor", value: 1.98 },
      { metric: "Sharpe Ratio", value: 1.45 },
      { metric: "Max Drawdown", value: -12.1 },
    ],
    startedAt: "2024-04-14 10:00",
    completedAt: "2024-04-14 18:45",
  },
  {
    id: "3",
    name: "MACD Cross Tuning",
    strategy: "MACD Cross",
    parameters: {
      fastEMA: "12",
      slowEMA: "26",
    ...
    bestResult: 1.67,
    results: [
      { metric: "Win Rate", value: 55.2 },
      { metric: "Profit Factor", value: 1.67 },
      { metric: "Sharpe Ratio", value: 1.12 },
      { metric: "Max Drawdown", value: -15.8 },
    ],
    startedAt: "2024-04-20 08:30",
  },
  {
    id: "4",
    name: "Bollinger Bands Sweep",
    ...
    bestResult: 0.45,
    results: [
      { metric: "Win Rate", value: 42.1 },
      { metric: "Profit Factor", value: 0.45 },
      { metric: "Sharpe Ratio", value: -0.25 },
      { metric: "Max Drawdown", value: -28.5 },
    ],
    startedAt: "2024-04-19 11:00",
    completedAt: "2024-04-19 15:20",
  },
  {
    id: "5",
    name: "Ichimoku Signals v2",
    strategy: "Ichimoku Signals",
    parameters: {
      tenkan: "9",
      kijun: "26",
      senkou: "52",
    ...
    bestResult: 2.12,
    results: [
      { metric: "Win Rate", value: 60.8 },
      { metric: "Profit Factor", value: 2.12 },
      { metric: "Sharpe Ratio", value: 1.56 },
      { metric: "Max Drawdown", value: -10.2 },
    ],
    startedAt: "2024-04-18 07:00",
    completedAt: "2024-04-18 22:15",
  },
];
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-xs font-medium uppercase mb-2">Total Experiments</p>
            <p className="text-2xl font-bold text-white">{experiments.length}</p>
          </div>
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-xs font-medium uppercase mb-2">Completed</p>
            <p className="text-2xl font-bold text-green-400">
              {experiments.filter((e) => e.status === "completed").length}
            </p>
          </div>
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-xs font-medium uppercase mb-2">Running</p>
            <p className="text-2xl font-bold text-blue-400">
              {experiments.filter((e) => e.status === "running").length}
            </p>
          </div>
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-xs font-medium uppercase mb-2">Best Profit Factor</p>
            <p className="text-2xl font-bold text-amber-400">
              {Math.max(...experiments.map((e) => e.bestResult)).toFixed(2)}
            </p>
          </div>
        </div>
            ...
            >
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <span className={`px-3 py-1 rounded text-xs font-semibold border ${getStatusColor(exp.status)}`}>
                ...
                <div className="border-t border-slate-700 bg-slate-800/20 p-6 space-y-6">
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 text-white font-semibold">{exp.startedAt}</span>
                    </div>
                    {exp.completedAt && (
                      <div>
                        <span className="text-slate-400">Completed:</span>
                        <span className="ml-2 text-white font-semibold">{exp.completedAt}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 pt-4 border-t border-slate-700">
                    <button className="px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 border border-blue-500/30 rounded text-sm font-semibold transition-all">
                      View Details
                    </button>
                    <button className="px-4 py-2 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/30 rounded text-sm font-semibold transition-all">
                      Fork Experiment
                    </button>
                  </div>
                </div>
