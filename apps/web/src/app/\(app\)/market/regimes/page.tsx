  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div
          className={`border rounded-lg p-6 ${overallColor.bg}`}
        >
          <div className="flex items-center gap-3">
            {overallColor.icon}
            <div>
              <p className="text-sm font-semibold text-slate-400 uppercase">Overall Market Regime</p>
              <p className={`text-2xl font-bold ${overallColor.text}`}>
                {formatRegimeName(overallRegime)}
              </p>
            </div>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      <td className="py-4 px-6">
                        <span className="font-bold text-white">{regime.symbol}</span>
                      </td>
                      <td className="py-4 px-6">
                        <div
                          className={`flex items-center gap-2 w-fit px-3 py-1 rounded border ${current.bg}`}
                        >
                          {current.icon}
                          <span className={`text-xs font-semibold ${current.text}`}>
                            {formatRegimeName(regime.currentRegime)}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6 text-right text-white font-semibold">
                        ${regime.price.toFixed(2)}
                      </td>
                      <td
                        className={`py-4 px-6 text-right font-semibold ${
                          regime.change >= 0 ? "text-green-400" : "text-red-400"
                        }`}
                      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {(["trending_up", "trending_down", "ranging", "volatile"] as const).map((regime) => {
            ...
            return (
              <div
                key={regime}
                className={`flex items-center gap-3 p-4 rounded border ${color.bg}`}
              >
                {color.icon}
                <div>
