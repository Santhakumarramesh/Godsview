        ...
        <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-6">
            ...
            <div className="inline-block min-w-full">
              ...
              <div className="flex gap-2 mb-2">
                ...
                {timeSlots.map((slot) => (
                  <div
                    key={slot}
                    className="flex-1 text-center text-xs font-semibold text-slate-400 py-2"
                  >
                    {slot}
                  </div>
                ))}
              ...
              {heatmapData.map((row, idx) => (
                <div key={idx} className="flex gap-2 mb-2">
                  <div className="w-28 text-sm font-semibold text-slate-300 py-2">
                    {row.session}
                  </div>
                  {row.times.map((value, timeIdx) => (
                    <div
                      key={timeIdx}
                      className={`flex-1 flex items-center justify-center py-4 rounded cursor-pointer transition-all hover:opacity-80 text-white font-semibold text-sm ${getHeatmapColor(
                        value
                      )}`}
          ...
          <div className="flex items-center gap-4 mt-6 pt-6 border-t border-slate-700">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">Moderate (40-55%)</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-yellow-500 rounded" />
              <span className="text-xs text-slate-400">Good (55-70%)</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-green-500 rounded" />
              <span className="text-xs text-slate-400">High (70-85%)</span>
            </div>
            <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <div className="flex justify-center">
                        <span
                          className={`px-3 py-1 rounded text-xs font-bold border ${getTradabilityColor(
                            liq.tradabilityScore
                          )}`}
                        >
                    ...
                    <td className="py-4 px-6 text-center">
                      <button
                        ...
                        className="px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs rounded transition-all"
                      >
        ...
        {selectedSymbol && (
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-6">
            <h3 className="text-lg font-bold text-white mb-4">
              Detailed Liquidity Analysis: {selectedSymbol}
            </h3>
