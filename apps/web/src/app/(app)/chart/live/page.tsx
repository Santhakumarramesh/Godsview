  ...
  const fetchData = useCallback(async (symbol: string, tf: string) => {
    ...
    try {
      const [barsData, quoteData] = await Promise.all([
        api.market.getBars(symbol, { timeframe: tf, limit: 200 }),
      ...
      const demoBars = Array.from({ length: 200 }, (_, i) => demoBar(Date.now() - (200 - i) * 900000));
      setBars(demoBars);
      setQuote({ symbol, price: 180.25, change: 1.5, volume: 50000000 });
    } finally {
      setLoading(false);
    }
  }, []);
