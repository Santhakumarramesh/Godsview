...
export default function EntryPlannerPage() {
  ...
  const fetchData = useCallback(async (symbol: string) => {
    ...
    setError("");
    try {
      const quoteData = await api.market.getQuote(symbol);
      setCurrentPrice(quoteData.price || 0);
      setEntryPrice(quoteData.price || 0);

      try {
        const confluenceData = await api.flow.getConfluenceZones(symbol);
      ...
      // Demo fallback
      setCurrentPrice(180.25);
      setEntryPrice(180.25);
      setConfluenceZones([
        { price: 180.50, type: "Support", strength: 85 },
        { price: 182.00, type: "Resistance", strength: 75 },
        { price: 179.00, type: "Support", strength: 65 },
      ]);
    } finally {
      setLoading(false);
    }
