  ...
  const fetchData = useCallback(async (symbol: string) => {
    ...
    try {
      const [htf, mtf, ltf] = await Promise.all([
        api.market.getBars(symbol, { timeframe: "1d", limit: 50 }),
        api.market.getBars(symbol, { timeframe: "1h", limit: 50 }),
        api.market.getBars(symbol, { timeframe: "15min", limit: 50 }),
      ]);
      ...
      const demoBars = Array.from({ length: 20 }, (_, i) => demoBar(Date.now() - (20 - i) * 86400000));
      setHtfData({ timeframe: "1d", bars: demoBars });
      setMtfData({ timeframe: "1h", bars: demoBars });
      setLtfData({ timeframe: "15min", bars: demoBars });
    } finally {
      setLoading(false);
    }
  }, []);
