...
export default function PositionsPage() {
  ...
  const fetchPositions = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await api.portfolio.getPositions({ status: 'open', limit: 50 })
      setPositions(data.trades || [])
      setLastRefresh(new Date())
    } catch (err) {
      // Demo fallback data
      setPositions([
        { id: '1', symbol: 'AAPL', side: 'buy', qty: 100, entry_price: 175.50, current_price: 180.25, pnl: 475, status: 'filled', opened_at: new Date(Date.now() - 3600000).toISOString() },
        { id: '2', symbol: 'TSLA', side: 'sell', qty: 50, entry_price: 245.80, current_price: 244.20, pnl: 80, status: 'filled', opened_at: new Date(Date.now() - 7200000).toISOString() },
        { id: '3', symbol: 'MSFT', side: 'buy', qty: 75, entry_price: 378.75, current_price: 380.00, pnl: 93.75, status: 'filled', opened_at: new Date(Date.now() - 10800000).toISOString() },
      ])
      setLastRefresh(new Date())
    } finally {
      setLoading(false)
