  ...
  useEffect(() => {
    const fetchHeatCards = async () => {
      try {
        ...
        const signalsData = await api.scanner.getSignals({ symbols, timeframe: "15min" });

        const symbolList = symbols.split(",");
        const quotes = await Promise.all(
          symbolList.map((sym) => api.market.getQuote(sym).catch(() => ({ symbol: sym, price: 0, change: 0, change_pct: 0 })))
        );
        ...
        const mockCards: HeatCard[] = [
          { symbol: "AAPL", price: 182.45, change: 2.34, changePct: 1.30, direction: "bull", score: 85.5, urgency: 0.92 },
          { symbol: "TSLA", price: 242.18, change: -3.45, changePct: -1.41, direction: "bear", score: 72.3, urgency: 0.78 },
          { symbol: "MSFT", price: 418.20, change: 1.23, changePct: 0.29, direction: "bull", score: 91.2, urgency: 0.95 },
        ];
        setItems(mockCards);
