...
export default function BacktestsPage() {
  ...
  const [backtestResult, setBacktestResult] = useState(mockBacktestResult);
  ...
  const handleRunBacktest = async () => {
    try {
      setLoading(true);
      setError(null);
      const result = await api.backtest.runBacktest({
      ...
      if (result) {
        setBacktestResult({
          ...
          netPnL: Math.round(result.totalReturn * 10000),
          avgHoldTime: "2h 34m",
        });
        setHasRun(true);
      }
      ...
      setError("Unable to run backtest");
      setHasRun(true);
    } finally {
      setLoading(false);
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div className="flex items-end">
              <button
                ...
                disabled={loading}
                className="w-full px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-semibold rounded flex items-center justify-center gap-2 transition-all disabled:opacity-50"
              >
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
              ...
              <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
                ...
                <p className="text-2xl font-bold text-blue-400">{backtestResult.profitFactor}</p>
              </div>
              <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
                <p className="text-slate-400 text-xs font-medium uppercase mb-1">Sharpe</p>
                <p className="text-2xl font-bold text-cyan-400">{backtestResult.sharpe}</p>
              </div>
              <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
                <p className="text-slate-400 text-xs font-medium uppercase mb-1">Max DD</p>
                <p className="text-2xl font-bold text-red-400">{backtestResult.maxDD}%</p>
              </div>
              <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
                <p className="text-slate-400 text-xs font-medium uppercase mb-1">Net P&L</p>
                <p className="text-2xl font-bold text-green-400">${backtestResult.netPnL.toLocaleString()}</p>
              </div>
              <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
                <p className="text-slate-400 text-xs font-medium uppercase mb-1">Avg Hold</p>
                <p className="text-lg font-bold text-amber-400">{backtestResult.avgHoldTime}</p>
              </div>
            </div>
