...
export default function QuantHomePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const resultsData = await api.backtest.getResults({ limit: 10 });
      const modelsData = await api.ml.getModels();
      const timeframesData = await api.backtest.getTimeframes();
      ...
      const mockModels: MLModel[] = [
        { id: '1', symbol: 'AAPL', type: 'RandomForest', accuracy: 0.82, status: 'active', created: new Date().toISOString() },
      ];
      const mockTimeframes = ['1m', '5m', '15m', '1h', '4h', '1d'];
