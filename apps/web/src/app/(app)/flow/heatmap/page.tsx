...
export default function HeatmapPage() {
  ...
  const fetchHeatmap = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const heatmapData = await api.flow.getHeatmap(symbol, { interval: '1min' }).catch(() => {
        // Mock fallback
        return {
          symbol,
          interval: '1min',
          levels: [
            { price: 150.0, bid_vol: 1000, ask_vol: 800, net: 200 },
            { price: 149.9, bid_vol: 900, ask_vol: 1100, net: -200 },
          ]
        }
      });
      setData(heatmapData);
