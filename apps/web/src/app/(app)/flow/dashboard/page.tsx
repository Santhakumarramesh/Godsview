...
export default function OrderFlowDashboardPage() {
  ...
  const [flowEvents, setFlowEvents] = useState(mockFlowEvents);
  ...
  useEffect(() => {
    const fetchFlowData = async () => {
      try {
        setLoading(true);
        const [imbalanceRes, absorptionRes] = await Promise.allSettled([
          api.flow.getFlowImbalance(selectedSymbol),
          api.flow.getFlowAbsorption(selectedSymbol),
        ]);

        if (imbalanceRes.status === "fulfilled" && imbalanceRes.value) {
          const imb = imbalanceRes.value;
          setMetrics((prev) => ({
            ...prev,
            imbalanceScore: Math.abs(imb.bidAskImbalance),
            delta: Math.round(imb.volumeImbalance),
          }));
        }

        if (absorptionRes.status === "fulfilled" && absorptionRes.value) {
          const abs = absorptionRes.value;
          setMetrics((prev) => ({
            ...prev,
            absorptionQuality: abs.largeOrderAbsorption,
          }));
        }
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-700 pb-6">
          ...
          <div className={`flex items-center gap-2 px-3 py-1 rounded text-xs font-semibold ${
            ...
            <div className={`w-2 h-2 rounded-full ${loading ? "" : "animate-pulse"} ${loading ? "bg-yellow-400" : "bg-green-400"}`} />
