...
export default function KillSwitchPage() {
  ...
  >(null);
  const [killSwitchStatus, setKillSwitchStatus] = useState("armed");
  ...
  useEffect(() => {
    const fetchKillSwitchStatus = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await api.risk.getKillSwitchStatus();
        if (response.success && response.data) {
          setKillSwitchStatus(response.data.status || "armed");
          setIsLiveMode(true);
        } else {
          setIsLiveMode(false);
        }
  ...
  const handleConfirm = async () => {
    try {
      setLoading(true);
      const response = await api.risk.activateKillSwitch();
      if (response.success) {
        setKillSwitchStatus("activated");
        setIsLiveMode(true);
      } else {
        setError("Failed to activate kill switch");
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Error activating kill switch");
    } finally {
        ...
        <div className="flex items-center justify-between border-b border-red-700/50 pb-6">
          ...
          <div className="text-right">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className={`font-bold text-lg ${killSwitchStatus === "armed" ? "text-green-400" : "text-red-400"}`}>
                {killSwitchStatus === "armed" ? "ARMED & READY" : "ACTIVATED"}
              </p>
            </div>
          </div>
        </div>
