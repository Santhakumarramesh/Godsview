...
export default function LearningLoopPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const backtestData = await api.backtest.getResults({ limit: 30 });
        const tradesData = await api.execution.getTrades({ status: 'all', limit: 30 });
        ...
        const mockTrades: Trade[] = [
          { id: '1', symbol: 'AAPL', pnl: 250, timestamp: new Date().toISOString() },
        ];
        setBacktests(mockBacktests);
        setTrades(mockTrades);
        deriveLessons(mockBacktests, mockTrades);
