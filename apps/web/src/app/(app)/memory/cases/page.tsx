...
export default function CaseLibraryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const signalsData = await api.memory.getSignalHistory({ limit: 50 });
        const tradesData = await api.execution.getTrades({ status: 'all', limit: 50 });
        ...
        const mockCases: CaseEntry[] = [
          { id: '1', symbol: 'AAPL', signal_direction: 'buy', outcome: 'win', confidence: 0.85, pnl: 250, timestamp: new Date().toISOString() },
          { id: '2', symbol: 'TSLA', signal_direction: 'sell', outcome: 'loss', confidence: 0.72, pnl: -150, timestamp: new Date().toISOString() },
        ];
        setCases(mockCases);
